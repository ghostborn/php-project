...
function check() {
    ...
    if (myform.txt_birthday.value == "") {
        alert("请输入您的生日");
        myform.txt_birthday.focus();
        return false;
    }
    if (CheckDate(myform.txt_birthday.value)) {
        alert("请输入标准日期（如：1980/05/29或1980-05-29）");
        myform.txt_birthday.focus();
        return false;
    }
    ...
    if ((i < 0) || (i - j > 0) || (j < 0)) {
        ...
        myform.txt_regemail.value = "";
        myform.txt_regemail.focus();
        return false;
    }
    if (myform.txt_province.value == "") {
        alert("请选择您所在的省会名称!");
        myform.txt_province.focus();
        return false;
    }
    if (myform.txt_city == "") {
        alert("请选择您所在的城市名称!");
        myform.txt_city.focus();
        return false;
    }
    if (myform.txt_ico == "") {
        alert("请选择您喜欢的人物头像!");
        myform.txt_ico.focus();
        return false;
    }
